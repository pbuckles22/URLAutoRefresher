...
1. **`main` is the integration branch.** Commit locally, run **`npm run ci`**, then push when green. For any **coherent slice** (feature, fix, or refactor worth recording), prefer a short-lived **`feature/<topic>`** branch first, then merge into **`main`** after CI is green — see [.cursor/skills/github-feature-workflow/SKILL.md](.cursor/skills/github-feature-workflow/SKILL.md). Trivial one-line typos can stay on **`main`** if you prefer.
...
**GitHub:** Optional PRs or **`gh`** still work if you want a diff view; they are **not** required to ship. **Agents:** default is **summaries + wait for OK** (step **5**); if the user chose the **PR gate**, push the branch, put summaries in the PR, merge only when they approve. Otherwise **local `npm run ci` green** → **commit** → **`npm run push:main`** (or push + **`npm run ci:watch-gh`**) → **GitHub CI green**; see [.cursor/skills/github-feature-workflow/SKILL.md](.cursor/skills/github-feature-workflow/SKILL.md).
