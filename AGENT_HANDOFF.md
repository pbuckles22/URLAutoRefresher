...
**Session handoff narratives** (reviews, CI, done / next) are **not** part of the published repo: use **`doc/handoff/HANDOFF-*.md`** (gitignored) or **`.cursor/handoff/handoff-*.md`** (gitignored), **and mirror the starting point in chat** per the handoff checklist. The **only** tracked file under `doc/handoff/` is **[HANDOFF_MOVE_TO_LOCAL.md](doc/handoff/HANDOFF_MOVE_TO_LOCAL.md)** (UNC → local disk, for contributors).
...
- **Version scheme (`MAJOR.EPIC.STORY`):** [doc/VERSIONING.md](doc/VERSIONING.md)
- **Session notes (gitignored depth + chat mirror):** see [.gitignore](.gitignore). Write the full narrative under **`doc/handoff/`** or **`.cursor/handoff/`**; **also mirror the starting point in chat** (checklist → _Chat response_). **Filename (recommended):** `doc/handoff/HANDOFF-YYYY-MM-DD-HHmmss.md` — **date + local time (24h)** in the name so multiple handoffs **per day** do not collide. Example: `HANDOFF-2026-04-17-143022.md`. Alternative: `.cursor/handoff/handoff-YYYY-MM-DD_HHmm.md`. **Inside the file:** put an ISO-style **Recorded** line under the title (e.g. `Recorded: 2026-04-17T14:30:22-05:00` or UTC) for humans and search.
...
- **Handoff (mandatory):** When the user wants a handoff, run code review (code-reviewer), tech debt (tech-debt-evaluator), and your **tests or coverage** as documented below; write a **local** handoff note **and mirror the full starting point in chat** (see [.cursor/rules/handoff-checklist.mdc](.cursor/rules/handoff-checklist.mdc) → _Chat response_). Do not record only in the gitignored file.
...
1. **`main` is the integration branch only — no direct commits.** **Do not `git commit` on `main`.** All work (features, fixes, docs, typos) happens on a short-lived **`feature/<topic>`** or **`fix/<topic>`** branch; **`main` advances only via merge** (local merge or GitHub merge when you use a PR). On the branch: implement, run **`npm run ci`**, commit, then merge into **`main`** when green — see [.cursor/skills/github-feature-workflow/SKILL.md](.cursor/skills/github-feature-workflow/SKILL.md). After merge, push **`main`** per step **3**.
2. **Before push:** Run **`npm run ci`** (lint, Prettier check, Vitest, build, Playwright E2E). Fix failures before pushing.
3. **After push to `main` (automated):** **`.husky/pre-push`** on pushes to **`origin/main`**: **`npm run ci`** → push → **`gh run watch --exit-status`**. Use normal **`git push origin main`** (or **`git push`** on **`main`**). Git may print **`failed to push some refs`** after success — the hook cancels the duplicate outer push; that message is **expected** if you see **`pre-push: automated push + GitHub CI watch complete`**. **`npm install`** also sets a local **`git alias.push`** (backup path). Manual: **`npm run push:main`**, **`npm run ci:watch-gh`**. Escape hatches: **`URLAR_SKIP_CI=1`**, **`URLAR_SKIP_GH_WATCH=1`**, **`git -c alias.push= push --no-verify origin main`**. Do not mark ship complete until **GitHub CI is green**.
4. **Before commit (version — first):** When the commit ships checked-off **[EDGE plan](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md)** scope (a story or epic closure), bump **`package.json`** `"version"` to **`MAJOR.EPIC.STORY`** per **[doc/VERSIONING.md](doc/VERSIONING.md)** (current shipped: **`0.16.1`** = Backlog **#8** overlay snap/drag). Run **`npm run build`** so **`manifest.json`** stays in sync. **Skip** for doc-only or typo commits with no story checkbox change. Include the version bump in the same commit as the shipped story when possible.
5. **Before commit (summaries + your OK):** After **`npm run ci`** is green, **before** `git commit` (and before push), use **[.cursor/skills/techwriter/SKILL.md](.cursor/skills/techwriter/SKILL.md)** for tone and structure. Treat the next two bullets as **two fixed audiences**—not one paragraph that tries to do both.
...
6. **Review (optional):** Use **code-reviewer**, **tech-debt-evaluator**, or **extension-architect** / **tech-lead** when useful—tools for you and agents, not a merge blocker.
7. **Lint + format:** **`npm run lint`** and **`npm run format:check`** are the first two steps of **`npm run ci`**. **`eslint-config-prettier`** disables ESLint rules that fight Prettier. After `npm install`, **Husky** runs **`lint-staged`** on **`git commit`** (Prettier write + ESLint `--fix` on staged files). For **change-aware** review, use **code-quality-gate** (`.cursor/skills/code-quality-gate/SKILL.md`) with **code-reviewer**. To install dependencies without Git hooks (e.g. tarball extract), set **`HUSKY=0`** for that `npm install`.

**GitHub (optional gate before `main`):** The repo does **not** require a PR to ship, but you **can** use GitHub so summaries appear **before** anything lands on **`main`**: push a **feature branch** (with commits), put the **same two summaries** (plan + regular user) in the **PR description** or first comment, and **do not merge** until you approve. Your “allow check-in” is then **Merge** on GitHub. That still allows commits on the branch; it adds review **before `main`**. For **no `git commit` at all** until you say so, use step **5**’s **chat OK** (agent stages, runs CI, posts summaries, waits for your **“commit”**).

**GitHub:** Optional PRs or **`gh`** still work if you want a diff view; they are **not** required to ship. **Agents:** default is **summaries + wait for OK** (step **5**); if the user chose the **PR gate**, push the branch, put summaries in the PR, merge only when they approve. Otherwise **local `npm run ci` green on the feature branch** → **commit on the branch** → **merge to `main`** → **`npm run push:main`** (or push + **`npm run ci:watch-gh`**) → **GitHub CI green**; see [.cursor/skills/github-feature-workflow/SKILL.md](.cursor/skills/github-feature-workflow/SKILL.md).

---

## Handoff protocol

When ending a session: (1) Run the handoff checklist (code review, tech debt, tests/coverage). (2) Update **[PM_PLAN.md](PM_PLAN.md)** and epic checkboxes in the **EDGE plan** if shipped scope changed — that is what **`main`** carries for **product** state. (3) Write a **local** session note under **`doc/handoff/`** (`HANDOFF-*.md`) and/or **`.cursor/handoff/handoff-*.md`** (both gitignored). Use a **timestamped filename** (`HANDOFF-YYYY-MM-DD-HHmmss.md`) and a **Recorded** line in the body when more than one handoff per day is possible. (4) Include Code review, Tech debt, Tests / CI, Done this session, Next up. (5) **Mirror the handoff in chat** — ship headline, Recorded path, Ship status table, dual-audience Done blurbs, and **Next up (from handoff)** so the next session can start without opening the gitignored file. Template: [.cursor/rules/handoff-checklist.mdc](.cursor/rules/handoff-checklist.mdc) → _Chat response_ and [.cursor/handoff/\_template.md](.cursor/handoff/_template.md). (6) **Update [always.mdc](.cursor/rules/always.mdc) Repo status** (branch, shipped version, in-progress slice, last handoff path) — checklist step **8**. Anything others must see on GitHub should land in **PM_PLAN**, the **EDGE plan**, or **README** — not only in gitignored handoff notes.
