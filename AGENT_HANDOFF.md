...
3. **Before commit (user-facing summary):** After **`npm run ci`** is green and you are ready to record the slice, give the user a short **“what changed”** in **business / plan language** — **before** `git commit` (and before push). Tie it to **[PM_PLAN.md](PM_PLAN.md)** and the **[EDGE plan](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md)** (epic and story names, outcomes, risks). Prefer outcomes for **users and maintainers** over file names, symbols, and test-infrastructure jargon unless the user asked for technical depth. One tight paragraph is enough; the diff carries implementation detail.
