...
3. **Before commit (summaries + your OK):** After **`npm run ci`** is green, **before** `git commit` (and before push):
   - Give a short **“what changed”** in **business / plan language** tied to **[PM_PLAN.md](PM_PLAN.md)** and the **[EDGE plan](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md)** (epic/story, outcomes, risks)—for maintainers and future you.
   - Add a second **plain-language** paragraph for a **regular user** (someone who only uses the extension): what they will or will not notice; no file names, APIs, or test jargon unless they asked for technical detail.
