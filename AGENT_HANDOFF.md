...
4. **Before commit (version — first):** When the commit ships checked-off **[EDGE plan](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md)** scope (a story or epic closure), bump **`package.json`** `"version"` to **`MAJOR.EPIC.STORY`** per **[doc/VERSIONING.md](doc/VERSIONING.md)** (current shipped: **`0.15.2`** = Epic **15** story **15.1** tech-debt patch). Run **`npm run build`** so **`manifest.json`** stays in sync. **Skip** for doc-only or typo commits with no story checkbox change. Include the version bump in the same commit as the shipped story when possible.
