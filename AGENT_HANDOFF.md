...
3. **After push to `main` (automated):** A local **`git config alias.push`** (installed on **`npm install`** via **`prepare`**) routes **`git push`** to **`main`** through **`Scripts/push-main-watch-ci.mjs`**: local **`npm run ci`** → push → **`gh run watch --exit-status`**. **`.husky/pre-push`** also runs **`npm run ci`** before direct pushes to **`main`** (bypass paths). Manual equivalents: **`npm run push:main`**, **`npm run ci:watch-gh`**. Escape hatches: **`URLAR_SKIP_CI=1`**, **`URLAR_SKIP_GH_WATCH=1`**, or **`git -c alias.push= push --no-verify`**. **Do not** mark ship complete until **GitHub CI is green** (or record failure explicitly).
