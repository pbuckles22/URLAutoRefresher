...
- **[Epic 10](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-10--url-first-membership-phased) — URL-first membership (phased):** make `targetUrl`/member key the durable identity; resolve live tabs at refresh; migrate schedule/pause keys off `tabId`; URL-centric UI + sweep; plus **[10.6 TwitchFavs](doc/requirements/twitch-favs-managed-membership.md)** (managed tab list for group name `TwitchFavs`). **10.1–10.4 done** — **10.4** adds URL-only [`TargetRef`](src/lib/types.ts), **`schemaVersion` 3** in [`state.ts`](src/lib/state.ts), dashboard/globals/individual flows + E2E; **10.1–10.3** retained member-key schedule/pause (`memberNextFireAt`, `pausedMemberKeys`), **`schemaVersion` 2 → 3** migration, alarms `urlar:gm:` ([`alarm-names.ts`](src/lib/alarm-names.ts)), [`scheduler.ts`](src/background/scheduler.ts). **Story order:** **10.5 → 10.6**; **10.6** after **10.1**, **recommended after 10.2** (see EDGE Epic 10 header). **Next default story:** **10.5**. **Backlog #7** is **superseded** by Epic 10 per EDGE plan.
