...
- **Epic 13:** **13.A1–13.A3** and **13.B1 shipped** — scheduler background modules + dashboard **shell** (`dashboard-shell` + `DashboardDom` / `DashboardContext`); **13.B2–13.B5** in plan; **G1–G5** in EDGE ([Parallel work](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#parallel-work--epics-11-13)).
...
- **[Epic 13](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-13--scheduler--dashboard-modularization):** **13.B1 shipped**; **13.B2 → … → 13.B5** next (serial on `dashboard-app.ts`, **G4**); **`npm run ci`** after each slice. ESLint / complexity: [.cursor/skills/code-quality-gate/SKILL.md](.cursor/skills/code-quality-gate/SKILL.md).
