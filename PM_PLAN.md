...
- **Epic 13:** **13.A1 shipped** — state alignment module [`src/background/scheduler-align-state.ts`](src/background/scheduler-align-state.ts) + Vitest; **13.A2–13.B5** in plan — scheduler extraction then dashboard modularization; **G1–G5** and lanes **L12 / L13A / L113** in EDGE ([Parallel work](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#parallel-work--epics-11-13)).
...
- **[Epic 13](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-13--scheduler--dashboard-modularization):** **13.A1 shipped**; **13.A2 → 13.A3** (then **13.B1 → … → 13.B5**); **`npm run ci`** after each slice. ESLint / complexity: [.cursor/skills/code-quality-gate/SKILL.md](.cursor/skills/code-quality-gate/SKILL.md).
