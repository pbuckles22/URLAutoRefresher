...
- **Epic 11:** **11.1–11.2, 11.4 done** — see EDGE [Epic 11](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-11--precision-volume-web-audio). **11.3** optional **L113**; **11.5–11.7** next (**G3**: one owner on [`dashboard-app.ts`](src/dashboard/dashboard-app.ts) for **11.5**).
...
- **[Epic 11](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-11--precision-volume-web-audio):** **11.4 shipped** — tab-routed `PRECISION_VOLUME_APPLY` + dashboard client; **11.3** per **L113** when staffed; **11.5–11.7** next — still **one PR at a time** on hot paths (**G3**). Requirements: [doc/requirements/precision-volume-controller.md](doc/requirements/precision-volume-controller.md).
