...
- **Epic 13:** **13.A1–13.A3 shipped** — scheduler modules under `src/background/` (`scheduler-align-state`, `scheduler-sync-alarms`, `scheduler-alarm-handlers`) + Vitest; **13.B1–13.B5** (dashboard modularization) in plan; **G1–G5** and lanes in EDGE ([Parallel work](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#parallel-work--epics-11-13)).
...
- **[Epic 13](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-13--scheduler--dashboard-modularization):** **Phase 13.A shipped** (**13.A1–13.A3**); **13.B1 → … → 13.B5** next; **`npm run ci`** after each slice. ESLint / complexity: [.cursor/skills/code-quality-gate/SKILL.md](.cursor/skills/code-quality-gate/SKILL.md).
