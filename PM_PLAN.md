...
- **[Epic 10](doc/plan/EDGE_URL_AUTO_REFRESHER_PLAN.md#epic-10--url-first-membership-phased) — URL-first membership (phased):** make `targetUrl`/member key the durable identity; resolve live tabs at refresh; migrate schedule/pause keys off `tabId`; URL-centric UI + sweep; **[10.6 TwitchFavs](doc/requirements/twitch-favs-managed-membership.md)** (reserved group name `TwitchFavs`: name-list patterns, prune/upsert targets, `tabs.onUpdated` sync). **10.1–10.6 done** — **10.4** adds URL-only [`TargetRef`](src/lib/types.ts), **`schemaVersion` 3** in [`state.ts`](src/lib/state.ts), dashboard/globals/individual flows + E2E; **10.1–10.3** retained member-key schedule/pause (`memberNextFireAt`, `pausedMemberKeys`), **`schemaVersion` 2 → 3** migration, alarms `urlar:gm:` ([`alarm-names.ts`](src/lib/alarm-names.ts)), [`scheduler.ts`](src/background/scheduler.ts). **10.5** (URL-first sweep: enrollment member-key overlap, badge URL refresh, overlay schedule API cleanup). **10.6** ([`src/lib/twitch-favs.ts`](src/lib/twitch-favs.ts), [`src/background/twitch-favs-sync.ts`](src/background/twitch-favs-sync.ts)). **Next default story:** **Epic 11** (precision volume) or backlog per EDGE plan. **Backlog #7** is **superseded** by Epic 10 per EDGE plan.
