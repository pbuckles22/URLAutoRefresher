...
- [ ] **10.6** — **TwitchFavs managed membership:** For the global group whose **name** is **`TwitchFavs`** (case-insensitive), treat **Auto-include URL patterns** as **streamer names** (newline or comma); expand to **`https://www.twitch.tv/{login}`**; **case-insensitive** matching; **prune** stored **`targets`** not on the list; **upsert** `TargetRef` rows (`targetUrl` per channel) on **`tabs.onUpdated`** and on save—when the same channel appears in a **new** tab, drop stale rows that no longer match the managed list (URL-first; no persisted `tabId` on targets). Vitest for parsers/sync; touch [`global-group-form.ts`](../../src/lib/global-group-form.ts), [`global-group-list-row.ts`](../../src/lib/global-group-list-row.ts), background (`tabs.onUpdated` + [`scheduler.ts`](../../src/background/scheduler.ts) alarm sync). **Requirements:** [doc/requirements/twitch-favs-managed-membership.md](../requirements/twitch-favs-managed-membership.md).
