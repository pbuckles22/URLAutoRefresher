...
12. **Twitch — auto-click channel points bonus** — When a **channel points bonus** claim UI appears on a watch page (e.g. “click to collect” / bonus chest), **automatically click** it so open TwitchFavs tabs do not miss the reward while multitasking. **Scope (TBD at epic):** Likely **TwitchFavs** tabs in enabled groups only; **opt-in** global or per-group pref (default off). **Out of scope:** Predicting bonus drops, redeeming rewards for channel point perks, or non-Twitch sites. **Pattern:** Reuse raid-guard / watch-layout approach — debounced DOM scan in a Twitch content script ([`twitch-raid-guard-runner.ts`](../../src/content/twitch-raid-guard-runner.ts), [`twitch-watch-layout.ts`](../../src/content/twitch-watch-layout.ts)); pure detection helpers in `src/lib/` for Vitest. **Risks:** Twitch markup and copy change often; bonus UI may overlap theater/chat or raid overlays — define click priority and dedupe (once per bonus instance). **Compliance:** Automation on pages the user already has open; review Twitch Terms of Service at epic design time. **Design deferred to epic:** Selectors / accessibility labels, wiring (extend bridge vs sibling script), pref key in [`prefs.ts`](../../src/lib/prefs.ts), optional overlay debug counter. **Likely touch:** `src/content/twitch-channel-points-bonus.ts`, `src/lib/twitch-channel-points-bonus.ts`, CI-safe Twitch stub E2E.
