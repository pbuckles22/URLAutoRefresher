...
| **13.B2** | **Individual list + jobs events:** `renderIndividualJobs`, `tickCountdowns` (partial), `bindJobsListEvents`, add-job form submit         | Shipped in [`dashboard-individual-jobs.ts`](../../src/dashboard/dashboard-individual-jobs.ts); `tickIndividualJobCountdowns` + globals still in `dashboard-app.ts` until **13.B5**.                                |
