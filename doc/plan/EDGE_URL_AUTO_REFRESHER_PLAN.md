...
- [x] **11.3** — **Dynamic media:** **`MutationObserver`** (or equivalent) attaches to **new** media elements after SPA navigation. _Outcome: YouTube-style navigations pick up new players._ _Implemented: [`precision-volume-media-mutation.ts`](../../src/lib/precision-volume-media-mutation.ts) (`gatherHtmlMediaUnderRoot`); [`precision-volume-bridge.ts`](../../src/content/precision-volume-bridge.ts) debounced **removal** observer (disconnect + `WeakMap`/`WeakSet` cleanup when `<video>` / `<audio>` leaves the DOM; **no** pre-hook on insert — zero-blast hook at gain 0 would mute playback until the user acts); Vitest: [`precision-volume-media-mutation.test.ts`](../../src/lib/precision-volume-media-mutation.test.ts). New SPA `<video>` / `<audio>` is picked up on the next shortcut/dashboard apply via existing `querySelectorAll` + `tryHookMediaElement`._

- [x] **11.4** — **Messaging:** Typed messages in [`src/lib/messages.ts`](../../src/lib/messages.ts); background routes **set gain** / mute to the correct tab’s content handler; use **ramped** gain updates. _Outcome: dashboard can drive live volume._ _Implemented: [`PRECISION_VOLUME_TAB_REQUEST`](../../src/lib/messages.ts) + [`precision-volume-tab-route.ts`](../../src/background/precision-volume-tab-route.ts); [`sendPrecisionVolumeTabRequest`](../../src/lib/precision-volume-tab-client.ts); content [`precision-volume-bridge.ts`](../../src/content/precision-volume-bridge.ts) (`set-linear-gain` + shortcut apply); Vitest: `precision-volume-tab-route.test.ts`, `precision-volume-tab-client.test.ts`._
...
- [x] **13.B2** — Individual list + jobs events: [`src/dashboard/dashboard-individual-jobs.ts`](../../src/dashboard/dashboard-individual-jobs.ts) (`renderIndividualJobs`, `tickIndividualJobCountdowns`, `bindJobsListEvents`, `bindAddIndividualJobForm`); shell `DashboardDom` extended for individual nodes; Tier 1 [`src/dashboard/dashboard-individual-jobs.test.ts`](../../src/dashboard/dashboard-individual-jobs.test.ts); [`dashboard-app.ts`](../../src/dashboard/dashboard-app.ts) delegates individual list + add form; live countdown wiring via **13.B5** [`dashboard-storage-sync.ts`](../../src/dashboard/dashboard-storage-sync.ts).
- [x] **13.B3** — Tab picker for individuals: [`src/dashboard/dashboard-individual-tab-picker.ts`](../../src/dashboard/dashboard-individual-tab-picker.ts) (tab cache, populate/filter select, target URL sync, UI bind); [`src/lib/preferred-pin-tab.ts`](../../src/lib/preferred-pin-tab.ts) (`resolvePreferredPinTabId`, `isSchedulableWebUrl`) shared with global tab browser; Tier 1 tests; `DashboardDom` includes job tab search + refresh.
- [x] **13.B4** — Global groups slice: [`src/dashboard/dashboard-global-groups.ts`](../../src/dashboard/dashboard-global-groups.ts) (`renderGlobalGroupsList`, `bindGlobalGroupsListEvents`, `bindGlobalGroupForm`, `renderGlobalTabBrowser`, `bindGlobalTabBrowserUi`, `bindGlobalTwitchFavsHint`, `tickGlobalGroupCountdowns`); shell `DashboardDom` extended for global nodes; Tier 1 [`src/dashboard/dashboard-global-groups.test.ts`](../../src/dashboard/dashboard-global-groups.test.ts); [`dashboard-app.ts`](../../src/dashboard/dashboard-app.ts) delegates globals bind/render + startup `Promise.all` (Tier 2 unchanged).
- [x] **13.B5** — Storage sync + tick interval: [`src/dashboard/dashboard-storage-sync.ts`](../../src/dashboard/dashboard-storage-sync.ts) (`wireDashboardStorageSync`, `tickDashboardCountdowns` — `chrome.storage.onChanged` for **`STORAGE_KEY`**, `onlyNonLayoutAppStateDiff` fast path vs full list refresh, 1s `setInterval`); Tier 1 [`src/dashboard/dashboard-storage-sync.test.ts`](../../src/dashboard/dashboard-storage-sync.test.ts); [`dashboard-app.ts`](../../src/dashboard/dashboard-app.ts) calls **`wireDashboardStorageSync`** early during init.

`initDashboardApp` composes **`dashboardContext`**, overlay prefs, storage sync, feature slices, cross-links, and initial **`Promise.all`**. Refactors keep the **single context object** pattern — extracted modules take **`DashboardContext`** (and caches where needed) instead of implicit outer scope. After Phase **13.B** slices: keep **Tier 2** [`e2e/epic-3-2.spec.ts`](../../e2e/epic-3-2.spec.ts), [`e2e/epic-4-3.spec.ts`](../../e2e/epic-4-3.spec.ts) green.

| Story     | Extract                                                                                                                                  | Notes                                                                                                                                                                                                                                                 |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **13.B1** | **Shell:** `wireCrossSurfaceLinks`, overlay pref block                                                                                   | Establishes `DashboardDom` / `DashboardContext` type pattern.                                                                                                                                                                                         |
| **13.B2** | **Individual list + jobs events:** `renderIndividualJobs`, `tickCountdowns` (partial), `bindJobsListEvents`, add-job form submit         | Shipped in [`dashboard-individual-jobs.ts`](../../src/dashboard/dashboard-individual-jobs.ts); countdown orchestration via **`tickDashboardCountdowns`** in [`dashboard-storage-sync.ts`](../../src/dashboard/dashboard-storage-sync.ts) (**13.B5**). |
| **13.B3** | **Tab browser + individual selectors:** `refreshCachedTabs`, `populateTabSelect`, filters, `syncIndividualTargetUrlFromSelectedTab`      | Shipped as [`dashboard-individual-tab-picker.ts`](../../src/dashboard/dashboard-individual-tab-picker.ts) + [`preferred-pin-tab.ts`](../../src/lib/preferred-pin-tab.ts); global edit tab rows use the same cache.                                    |
| **13.B4** | **Global groups:** `renderGlobalGroupsList`, `bindGlobalGroupsListEvents`, global form submit, `renderGlobalTabBrowser` / search filters | Shipped in [`dashboard-global-groups.ts`](../../src/dashboard/dashboard-global-groups.ts); no **13.B4a** / **13.B4b** split (single slice).                                                                                                           |
| **13.B5** | **Storage listener + interval tick wiring** in e.g. `dashboard-storage-sync.ts`                                                          | Shipped — **`wireDashboardStorageSync`** / **`tickDashboardCountdowns`**; **`onlyNonLayoutAppStateDiff`** vs **`renderIndividualJobs`** / **`renderGlobalGroupsList`**; avoid circular imports.                                                       |
...
8. **Page overlay — position: avoid blocking host UI (left/right vs drag)** — _(Shipped on `main` at **`0.16.1`**.)_ **UX:** **⇄** snap toggles top-right ↔ top-left; **⋮⋮** drag handle repositions anywhere (clamped to viewport). **Persistence:** global pref `overlayPosition` in [`prefs.ts`](../../src/lib/prefs.ts) — remembered across refresh and tabs. Default remains top-right. **Tier 1:** [`overlay-position.ts`](../../src/lib/overlay-position.ts); **Tier 2:** [`e2e/extension.spec.ts`](../../e2e/extension.spec.ts) (snap + reload).

9. **Precision volume — default to active (focused) tab** — _(Shipped on `main` at **`0.16.2`**.)_ Dashboard/side panel fader and **0%** apply to the **active http(s) tab** in the last-focused window by default (same mental model as shortcuts); when the extension UI is focused, falls back to the first schedulable tab in that window via [`resolvePreferredPinTabId`](../../src/lib/preferred-pin-tab.ts). Optional **Tab (override)** picker remains. **Touch:** [`dashboard-precision-volume.ts`](../../src/dashboard/dashboard-precision-volume.ts), [`precision-volume-target-tab.ts`](../../src/lib/precision-volume-target-tab.ts), [`prefs.ts`](../../src/lib/prefs.ts) (`lastTabId` null = active-tab mode). **Tier 1/2:** [`precision-volume-target-tab.test.ts`](../../src/lib/precision-volume-target-tab.test.ts), [`e2e/epic-11-9-active-tab-precision-volume.spec.ts`](../../e2e/epic-11-9-active-tab-precision-volume.spec.ts).
...
12. **Twitch — auto-click channel points bonus** — _(Shipped on `main` at **`0.16.5`**.)_ When a **channel points bonus** claim UI appears on a TwitchFavs **home** tab (enabled group), **automatically click** Claim Bonus so users do not miss collectable rewards. **Opt-in** global pref `twitchChannelPointsBonusEnabled` in [`prefs.ts`](../../src/lib/prefs.ts) (default **off**); dashboard Twitch section checkbox. **Out of scope:** Bonus prediction, redeeming perks, non-Twitch pages. **Implementation:** Pure DOM helpers [`twitch-channel-points-bonus.ts`](../../src/lib/twitch-channel-points-bonus.ts); debounced runner [`twitch-channel-points-bonus-runner.ts`](../../src/content/twitch-channel-points-bonus-runner.ts) wired in [`twitch-live-bridge.ts`](../../src/content/twitch-live-bridge.ts); background armed push mirrors Epic 14 raid guard ([`twitch-channel-points-bonus.ts`](../../src/background/twitch-channel-points-bonus.ts)). Dedupe: one click per visible bonus instance; rate-limit repeat clicks. **Tier 1:** lib + runner + armed tests; **Tier 2:** [`e2e/backlog-12-channel-points-bonus.spec.ts`](../../e2e/backlog-12-channel-points-bonus.spec.ts). **Manual:** Real Twitch spot-check when bonus UI appears (stub may not match live DOM). **Risks:** Twitch selector/copy churn — extend [`twitch-channel-points-bonus.ts`](../../src/lib/twitch-channel-points-bonus.ts) selectors as needed.
