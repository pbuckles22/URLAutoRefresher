...
| **PVC.11** | **Active-tab dashboard control (backlog #9):** Dashboard/side panel volume applies to the **focused active tab** by default (same mental model as shortcuts), not only after manual **Target tab** selection. Optional override picker may remain. | **0%** mutes what you are watching.                |
